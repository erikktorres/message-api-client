...
module.exports = function(groupsHostGetter, messagesHostGetter, request) {
  ...
  return {
    getThreadMessages: function(parentMessageId, sessionToken, cb) {

      withMessagesApiHost(cb, function(apiHost){

        var options = {
          url: apiHost + '/thread/'+parentMessageId,
          method: 'get',
          headers: {
    ...
    addToThread: function(message, parentMessageId, sessionToken, cb) {

      withMessagesApiHost(cb, function(apiHost){

        var options = {
          url: apiHost + '/reply/'+parentMessageId,
          method: 'post',
        ...
        request(options, function (err, res, body) {
          ...
          if (res.statusCode == 200) {
            ...
            return cb({ message: util.format('No id returned or bad statusCode[%s]', res.statusCode), statusCode: 503 });
          }
        });
      });

    },
    startNewThread: function(message, groupId, sessionToken, cb) {

      withMessagesApiHost(cb, function(apiHost){

        var options = {
          url: apiHost + '/send/'+groupId,
          method: 'post',
          params : { message : message},
          headers: {
            'X-Tidepool-Session-Token': sessionToken
